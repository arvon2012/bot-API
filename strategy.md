...
|止损价格|float|否|到达价格触发清仓操作，默认传0|
|防暴跌开关|int|否|是否打开防暴跌功能，1 打开 0 关闭|
|防暴跌检测时间周期|string|否|可选参数: 15m(15分钟) 1h(1小时) 4h(4小时) 1d(1天)|
|防暴跌跌幅|float|否|当发现盘面出现该跌幅, 机器人暂停并稍后重启. 例如设置10%, 需要传参 0.1|
|解套模式|int|否|是否打开解套功能，1 打开 0 关闭, 开启后, 机器人会在每次止盈时候自动尝试解套高位套牢单|
